...
def create_builder_template(info: utils.DatasetInfo):
  """Calls the required (possibly format-specific) dataset builder template.

  Args:
    info: `utils.DatasetInfo` object containing all information about the
...
def _standard_template(info: utils.DatasetInfo) -> str:
...
def _conll_template(info: utils.DatasetInfo) -> str:
